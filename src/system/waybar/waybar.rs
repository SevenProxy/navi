...
pub fn Waybar() -> Html {
    ...
    html! {
        <header class="w-full h-fit p-0 relative z-20">
            if *window_start {
                <div class="px-2 py-2 absolute z-40 top-8 left-o bg-pink-300">
                    <div class="w-full h-full">
                        <ul class="flex items-start flex-col gap-2">
                            { for get_menu_select().iter().map(|v| html! {
                                <li key={v.name.clone()} class="flex items-center justify-between max-w-[200px] min-w-[200px] relative">
                                    <div class="flex items-center gap-2">
                                        {v.icon.clone()}
                                        <p class="font-bold text-base">{v.name.clone()}</p>
                                    </div>
                                    <svg fill="currentColor" class="bi bi-chevron-right w-4 h-4 text-black" viewBox="0 0 16 16">
                                        <path fill-rule="evenodd" d="M4.646 1.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1 0 .708l-6 6a.5.5 0 0 1-.708-.708L10.293 8 4.646 2.354a.5.5 0 0 1 0-.708"/>
                                    </svg>
                                    <button class="absolute top-0 left-0 w-full h-full"></button>
                                        ...
                                        <ul class="flex flex-col items-start gap-2">
                                            { for get_apps().iter().map(|v| html! {
                                                <li class="flex items-center gap-2 relative" key={v.name.clone()}>
                                                    ...
                                                    <p class="text-bold text-base">{v.name.clone()}</p>
                                                    <button class="absolute top-0 left-0 w-full h-full"></button>
                                                </li>
                                            })}
                                        </ul>
